      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          ...
          <a-form-item label="菜单类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-radio-group @change="onChangeMenuType" v-decorator="['type',{'initialValue':localMenuType}]">
          ...
          <a-form-item
            ...
            :label="menuLabel"
            hasFeedback>
            <a-input placeholder="请输入菜单名称" v-decorator="[ 'title', validatorRules.name]" :readOnly="disableSubmit"/>
          </a-form-item>
          ...
          <a-form-item
            ...
            :wrapperCol="wrapperCol"
            label="菜单路径">
            <a-input placeholder="请输入菜单路径" v-decorator="[ 'path',validatorRules.url]" :readOnly="disableSubmit"/>
          </a-form-item>
          ...
          <a-form-item
            ...
            :wrapperCol="wrapperCol"
            label="前端组件">
            <a-input placeholder="请输入前端组件" v-decorator="[ 'component',validatorRules.component]"
          ...
          <a-form-item
            ...
            :wrapperCol="wrapperCol"
            label="授权标识">
            <a-input placeholder="多个用逗号分隔, 如: user:list,user:create" v-decorator="[ 'authority', {}]"
                     :readOnly="disableSubmit"/>
          </a-form-item>
          ...
          <a-form-item
            ...
            :wrapperCol="wrapperCol"
            label="排序">
            <a-input-number placeholder="请输入菜单排序" style="width: 200px" v-decorator="[ 'seq',validatorRules.sortNo]"
                            :readOnly="disableSubmit"/>
          </a-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(['AddMenu', 'ModifyMenu']),
      add () {
        // 默认值
        this.edit({ parentId: 0, route: true, action: 'add' })
      },
      edit (record) {
        this.resetScreenSize() // 调用此方法,根据屏幕宽度自适应调整抽屉的宽度
        this.form.resetFields()
        this.model = Object.assign({}, record)
        this.model.parentId = String(record.parentId)
        //--------------------------------------------------------------------------------------------------
        //根据菜单类型，动态展示页面字段
        ...
        this.show = record.type == 2 ? false : true
        this.menuLabel = record.type == 2 ? '按钮/权限' : '菜单名称'
        ...
        let fieldsVal = pick(this.model, 'title', 'authority', 'component', 'path', 'seq', 'type')
        this.$nextTick(() => {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (this.type === 1) {
              console.log('添加')
               this.AddMenu(formData).then(res => {
                 this.notice(res)
               }).catch(err => {
                 console.log(err)
               }).finally(() => {
              ...
              console.log('修改')
               this.ModifyMenu(formData).then(res => {
                 this.notice(res)
               }).catch(err => {
                 console.log(err)
               }).finally(() => {
