...
const constantRouterComponents = {
  ...
  // list
  'MenuList': () => import('@/views/other/PermissionList'),
  'RoleList': () => import('@/views/other/RoleList'),
  'bms_manager_log': () => import('@/views/other/LogList'),
  'system': PageView,
 ...
 */
export let generatorDynamicRouter
...
export const generator = (routerMap, parent) => {
  return routerMap.map(item => {
    const { title, show, hideChildren, hiddenHeaderContent, target, icon } = item.meta || item
    const currentRouter = {
      ...
      meta: { title: title, icon: icon || undefined, hiddenHeaderContent: hiddenHeaderContent, target: target, keepAlive: false}
    }
    // 是否设置了隐藏菜单
    if (show === false) {
